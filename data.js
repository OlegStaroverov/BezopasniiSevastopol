/* data.js
   Единое хранилище и бизнес-логика мини-приложения
   Используется:
   - app.js
   - admin-panel.js
*/
...
(() => {
  ...
  const getLS = (key, fallback = null) => {
    try {
      ...
      return v === null ? fallback : safeJSON.parse(v, fallback);
    } catch (_) {
      return fallback;
    }
  };
  ...
  const getTheme = () => getLS(THEME_KEY, "dark");
  ...
  const validatePhone = (phone) => {
    ...
    return /^[0-9+\-\s()]{6,20}$/.test(String(phone));
  };
  ...
  const normalizeType = (type) => {
    if (type === "wifi_problem" || type === "wifi_suggestion") {
      return "wifi";
    }
    return type;
  };
  ...
  const saveReport = async (type, payload) => {
    ...
    if (!REPORT_KEYS[t]) return false;

    const list = getLS(REPORT_KEYS[t], []);
    ...
    return setLS(REPORT_KEYS[t], list);
  };

  const getReports = async (type) => {
    const t = normalizeType(type);
    if (!REPORT_KEYS[t]) return [];
    return getLS(REPORT_KEYS[t], []);
  };
...
// Данные точек Wi-Fi (упрощенная версия)
  ...
  const findNearestWifi = (origin, limit = 10, maxRadius = Infinity) => {
    if (!origin || !origin.lat || !origin.lon) return [];

    return WIFI_POINTS
      .map((p) => ({
        ...p,
        distance: distance(origin, { lat: p.lat, lon: p.lon })
      }))
  ...
  const searchWifi = (query) => {
    const q = String(query || "").toLowerCase().trim();
    if (!q) return WIFI_POINTS;

    return WIFI_POINTS.filter(
      (p) =>
