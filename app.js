...
// binds
        ...
        safe(() => this._bindHomeActions(), "bind.home");
    ...
    _bindHomeActions() {
      const wrap = document.getElementById("homeActions");
      if (!wrap) return;
  ...
  В Севастополе действует система видеонаблюдения «Аргус», включающая более 2000 камер по всему городу. Система используется для контроля общественного порядка, помощи правоохранительным органам, выявления правонарушений и поиска пропавших людей. Если вы стали свидетелем происшествия или считаете, что ситуация требует внимания, вы можете отправить обращение через эту форму. Сообщение будет зарегистрировано и передано ответственным специалистам.
        ...
        <form id="argusForm" class="glass-card form-card" autocomplete="on">
          <div class="form-grid">
            ...
            <div class="form-group form-group--full">
              <label>Хотите приложить медиа?</label>
              <div class="segmented segmented-2" role="radiogroup" aria-label="Хотите приложить медиа?">
                <label class="seg-btn">
                  <input type="radio" name="argusMediaChoice" value="yes" />
                  <span>Да</span>
                </label>
                <label class="seg-btn">
                  <input type="radio" name="argusMediaChoice" value="no" checked />
                  <span>Нет</span>
                </label>
              </div>
              <div class="form-hint">Файлы не загружаются — в обращение передадим только отметку: да/нет.</div>
            </div>
          </div>
    ...
    _bindArgusForm() {
      ...
      // медиа в Аргусе: только отметка Да/Нет (без загрузки файлов)
      ...
      form.addEventListener("submit", async (e) => {
        ...
        const payload = this._collectCommonPayload({
          ...
          mediaChoiceName: "argusMediaChoice",
        ...
        this._showLoading("Записываем ваше обращение…");
        let saved = false;
        try {
          saved = await AppData.saveReport("argus", report);
        } finally {
          this._hideLoading();
        }
        ...
        this._syncLocationUI("argus");
        // превью медиа отсутствует (только отметка Да/Нет)
    ...
    _bindAppointmentForm() {
      ...
      form.addEventListener("submit", async (e) => {
        ...
        let saved = false;
        try {
          saved = await AppData.saveReport("appointment", report);
        } finally {
          this._hideLoading();
        }
        if (!saved) { this.toast("Не удалось сохранить обращение", "danger"); this.haptic("error"); return; }
    ...
    _showLoading(text = "Записываем ваше обращение…") {
    ...
    _bindSecurityForm() {
      ...
      form.addEventListener("submit", async (e) => {
        ...
        const payload = this._collectCommonPayload({
          ...
          mediaChoiceName: "securityMediaChoice",
        ...
        let saved = false;
        try {
          saved = await AppData.saveReport("security", report);
        } finally {
          this._hideLoading();
        }
        if (!saved) { this.toast("Не удалось сохранить обращение", "danger"); this.haptic("error"); return; }
    ...
    _bindGraffitiForm() {
      ...
      form.addEventListener("submit", async (e) => {
        ...
        const payload = this._collectCommonPayload({
          ...
          mediaChoiceName: "graffitiMediaChoice",
        ...
        let saved = false;
        try {
          saved = await AppData.saveReport("graffiti", report);
        } finally {
          this._hideLoading();
        }
        if (!saved) { this.toast("Не удалось сохранить обращение", "danger"); this.haptic("error"); return; }
  ...
  box.appendChild(frag);
    ...
    _bindWifiProblemForm() {
      ...
      form.addEventListener("submit", async (e) => {
        ...
        let saved = false;
        try {
          saved = await AppData.saveReport("wifi_problem", report);
        } finally {
          this._hideLoading();
        }
        if (!saved) { this.toast("Не удалось сохранить обращение", "danger"); this.haptic("error"); return; }
    ...
    _bindWifiNewForm() {
      ...
      form.addEventListener("submit", async (e) => {
        ...
        let saved = false;
        try {
          saved = await AppData.saveReport("wifi_suggestion", report);
        } finally {
          this._hideLoading();
        }
        if (!saved) { this.toast("Не удалось сохранить обращение", "danger"); this.haptic("error"); return; }
    ...
    _collectCommonPayload({ nameSel, phoneSel, emailSel, descriptionSel, location, mediaChoiceName }) {
      ...
      // Медиа: в нужных формах мы показываем только вопрос "Хотите приложить медиа?" (Да/Нет)
      let media = "";
      if (mediaChoiceName) {
        const picked = document.querySelector(`input[name="${mediaChoiceName}"]:checked`);
        const v = String(picked?.value || "").toLowerCase();
        media = v === "yes" ? "yes" : (v === "no" ? "no" : "no");
      }
    ...
    async _notifyAdmins(type, report) {
      // Email service removed by requirement (no отправки на email).
      // Emails are still stored in config for future use.
      return;
    }
  }
