...
/* calendar: prevent mobile overscroll jitter */
.dp, .dp * {
...
.dp__grid {
  ...
  touch-action: manipulation; /* не даём браузеру пытаться скроллить/зумить при тапах */
}

/* days from other months: slightly dim, but clickable if not disabled */
.dp__cell.is-outside:not(.is-disabled):not(:disabled) {
  opacity: .70;
}

/* calendar: nav buttons center icon */
.dp__nav {
